...
## 16. Pentalateral IAH — Nós e Conexões (2026-05-18)
...
| `PASSO7_EMBAIXADOR` | Template | Template para briefing pré-reunião, debrief pós-reunião e pipeline de lead |
| `AVISO_EMBAIXADOR` | Documento | Anti-padrões de relacionamento com cliente (5 padrões + vetos) |
| `ir_ao_embaixador_ps1` | Script | Ativa o Embaixador: copia MENSAGEM_INTERACAO_INICIAL + abre browser + Explorer |

### Conexões

```
Pentalateral_IAH
    ├── Músculo          [papel: construtor, executor]
    ├── Estrategista     [papel: visão de mercado, DIRETRIZ]
    ├── Auditor          [papel: auditoria histórica, Skill]
    ├── Embaixador       [papel: inteligência persistente do cliente]
    └── Diretor          [papel: originador soberano, veredito]

Embaixador
    ├── MEMORIA_EMBAIXADOR    [gera e atualiza]
    ├── P_031                 [governa: CONFIRMA/EXPANDE/ALERTA]
    ├── P_032                 [governa: atualização automática pelo Músculo]
    ├── PASSO7_EMBAIXADOR     [template de ativação]
    ├── AVISO_EMBAIXADOR      [anti-padrões que monitora]
    └── ir_ao_embaixador_ps1  [script de ativação]

MEMORIA_EMBAIXADOR
    ├── CLIENTES/[NOME]/CLAUDE_PROJECT/   [localização]
    ├── Embaixador                         [quem atualiza primariamente]
    ├── Músculo                            [quem atualiza via P-032]
    └── Loop_Evolutivo                     [alimenta a cada debrief]

P_031
    ├── Embaixador       [quem executa]
    ├── [M-1..M-5]       [ideias do Músculo que recebem CONFIRMA/EXPANDE/ALERTA]
    ├── [G-1..G-5]       [ideias do Estrategista que recebem CONFIRMA/EXPANDE/ALERTA]
    └── [N-1..N-5]       [ideias do Auditor que recebem CONFIRMA/EXPANDE/ALERTA]

P_032
    ├── Músculo          [quem executa: atualiza MEMORIA_EMBAIXADOR sem intervenção do Diretor]
    └── MEMORIA_EMBAIXADOR [o que atualiza]
```

### Volume de Ideias por Ciclo

```
[M-1 a M-5]  — Músculo (perspectiva técnica do construtor)
[E-1 a E-5]  — Embaixador (perspectiva do relacionamento real com cliente)
