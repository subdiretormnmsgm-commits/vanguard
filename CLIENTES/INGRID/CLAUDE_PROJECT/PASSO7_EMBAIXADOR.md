# PASSO 7 — EMBAIXADOR (CLAUDE PROJECTS) · PROJETO INGRID · LOOP 7
> Instância do PASSO7_EMBAIXADOR_TEMPLATE.md · Atualizado em 2026-05-30
...
Data: 2026-05-30
Loop: 7 | Fase: REAÇÃO AO PENTALATERAL
Modo: COMPLETO
```
...
**Banca:** Instituto Quadrix | **Data prova:** 2026-09-06 | **Dias até a prova:** ~99
**Camada:** 3 — SaaS Readiness | **Loop atual:** 7
**Temperatura atual:** VERDE SUSTENTADO — Score 7.5/10
**App:** https://subdiretormnmsgm-commits.github.io/vanguard/ — LIVE v20

**Gates aprovados (todos os 15 dias):**
- Dia 2 ✅ · Dia 5 ✅ · Dia 8 ✅ · Dia 11 ✅ · Dia 15 ✅ (Ingrid admin Supabase — 2026-05-30)

**Banco:** 460+ questões · 13 disciplinas · Supabase da Ingrid (yjqvjhezwhepwomukudt)

**Estado pós-Loop 6 (v20):**
- F-1 Saudação Noturna · F-2 Distração Vingativa Silenciosa · F-5 Modo Véspera (ativar 2026-08-30)
- F-7 Raio-X SVG + Brasão · F-8 Termômetro da Aprovação — ATIVOS
- F-4 Gatilho 19h45 + F-6 Relatório Semanal WhatsApp — código entregue, **deploy CLI PENDENTE**
- DADOS-WATCH: VERDE — 102 respostas · 1 user_id · SM-2 íntegro
- LEGAL-WATCH: VERDE · P-013 VERDE (soberania confirmada 2026-05-30)
- GitHub Pages push: BLOQUEADO — token no histórico (Eduardo: seguir link GitHub Security)

**Perfil comportamental confirmado:**
- "Gostei bastante. Amanhã volto para atacar mais." (2026-05-24) — shift de estudante para player (P-079)
- Lê enunciados de forma literal — notou negrito ausente na Q18
- Abandona silenciosamente quando frustrada — nunca reclama
- Horário de pico: noturno (~20h confirmado verbatim)
- Pipeline de referral: zero (não conhece candidatos) — próxima janela pós-aprovação

**Watchdog ativo:**
- [CHURN-WATCH]: DESATIVADO — temperatura 7.5/10 sustentada
- [SCOPE-WATCH]: IMPROVÁVEL
- Pitch SaaS R$97/mês: janela aberta — aguarda Ingrid verbalizar progresso
...
- [x] SEÇÃO D — REAÇÃO AO PENTALATERAL (DIRETRIZ V7 + ingrid-v7.md — Loop 7 · 2026-05-30)
...
> DIRETRIZ V7 (Gemini) + ingrid-v7.md (NotebookLM) · Loop 7 · 2026-05-30
> Contexto: Todos os gates aprovados · App v20 live · SaaS Readiness em foco · Deploy CLI pendente.

```
Embaixador, reação ao ciclo atual do Pentalateral — INGRID.
Loop 7 · Sessão 2026-05-30 · SaaS Readiness + Pipeline Comercial

ESTADO ATUAL:
- v20 live. Gate Dia 15 aprovado 2026-05-30 (Ingrid admin Supabase própria — P-013 VERDE).
- LEGAL-WATCH VERDE · DADOS-WATCH VERDE (102 respostas · 1 user_id).
- Temperatura 7.5/10 VERDE SUSTENTADO — "Gostei bastante. Amanhã volto para atacar mais." (2026-05-24).
- F-4 (cron 19h45) e F-6 (relatório semanal) com código entregue mas deploy CLI pendente.
- GitHub Pages push bloqueado — token no histórico (Eduardo: seguir link GitHub Security).
- Pitch SaaS R$97/mês: janela aberta — aguarda Ingrid verbalizar progresso.

[M-1 a M-5] — IDEIAS DO MÚSCULO (Loop 6 · relatorio_V6_INGRID):

M-1: Painel de Uso Real para Eduardo (não para Ingrid)
     Dashboard interno: Ingrid abriu o app hoje? Qual feature tocou? Quanto tempo ficou? F-7 teve cliques? F-8 foi visualizado? Eduardo enxerga comportamento real sem perguntar.
     Pergunta: O comportamento real de Ingrid confirmaria ou contradiz o que Eduardo imagina?

M-2: Gatilho de Reativação Silenciosa após 5 dias sem sessão
     Se Ingrid não abre o app por 5 dias corridos, sistema envia automaticamente: "Você estava indo muito bem — faltam N dias para a prova. Só 10 questões hoje."
     Pergunta: 5 dias sem uso é churn real para o perfil da Ingrid ou período normal de pausa?

M-3: Score de Consistência Semanal (não de acerto)
     Quantos dias da semana Ingrid fez pelo menos 5 questões? Exibido no relatório: "Você foi consistente em X/7 dias." Consistência prediz aprovação melhor que acerto isolado.
     Pergunta: Ingrid prefere ser medida por consistência ou por nota?

M-4: Link de Demonstração Anônimo para Prospects
     URL de leitura pública (sem login) com dashboard anonimizado de Ingrid. Números reais — argumentos de venda com prova social concreta. ALERTA: exibir só gráficos/Heatmap — NUNCA conteúdo das questões (risco IP).
     Pergunta: Ingrid se sentiria confortável com seus dados de performance sendo mostrados (anonimizados)?

M-5: Audit Trail de Churn Risk — Alerta Compound no Telegram
     Se: Ingrid sem sessão 3+ dias + cron falhou + relatório não enviado → Telegram Eduardo: "CHURN RISK — 3 sinais simultâneos."
     Pergunta: Ingrid costuma sumir por múltiplos dias sem aviso ou ausências dela têm padrão previsível?

---

[G-1 a G-5] — IDEIAS DO ESTRATEGISTA (GEMINI · DIRETRIZ V7 · 2026-05-30):

G-1: Safe-Horizon SM-2 Adaptativo
     Se data da prova atrasar para 2027, sistema aciona modo manutenção — expande SM-2 e reduz feed para 5 questões/dia. Evita esgotamento da usuária.
     ALERTA AUDITOR: VETADO como prioridade — alvo é 06/09/2026. Implementar só como flag desabilitada.
     Pergunta: Ingrid reagiria a mudança de ritmo sem comunicação explícita?

G-2: Dry-Run de Isolamento de Tenant (test_tenant_isolation.ps1)
     Script simula injeção maliciosa com user_id alterado — valida fisicamente que RLS bloqueia o vazamento antes de abrir para segundo cliente.
     Pergunta: A Embaixadora vê risco comercial no processo de auditoria técnica que o Músculo não está vendo?

G-3: Mapeador de Replicabilidade Quadrix
     Função analítica que cruza edital_sedes.json com editais abertos do DF (mesma banca) — aponta qual próximo cargo tem 80%+ similaridade programática.
     ALERTA AUDITOR: Desvio de foco para Loop 7. Usar como análise offline para pipeline do Embaixador.
     Pergunta: Ingrid mencionou ou tem contato com outros candidatos Quadrix?

G-4: Telemetria Passiva por Evento de Componente
     Listener assíncrono no PWA registra cliques em F-7 (Raio-X) e F-8 (Termômetro) na tabela evento_uso. Zero trackers de terceiros — soberania de dados total.
     Pergunta: A Embaixadora tem hipótese sobre quais features Ingrid efetivamente usa vs. ignora?

G-5: Git Hook pre-push Antiamnésia de Processo (P-045)
     Hook intercepta push e bloqueia se WIP_BOARD mudou sem MEMORIA + relatorio correspondentes em HISTORICO.
     Pergunta: Ferramenta de processo — sem impacto direto em Ingrid.

---

[N-1 a N-5] — IDEIAS DO AUDITOR (NOTEBOOKLM · ingrid-v7.md · 2026-05-30):

N-1: Telemetria de Vínculo Contratual Acoplada (P-023)
     Painel de Uso (M-1) exibe ícone permanente [✅ LEGAL-WATCH] buscando flag Termo_Assinado no banco. Compliance visível no painel do Diretor.
     Pergunta: A Embaixadora vê risco jurídico ainda ativo com Ingrid que o Músculo subestimou?

N-2: Interceptor de RLS Silencioso no Front-end
     Fetch wrapper captura erros 403 pós-RLS e faz reload passivo do JWT — Ingrid nunca vê tela branca ou "zero questões".
     Pergunta: Ingrid já relatou alguma vez que o app "sumiu" ou "não carregou"?

N-3: Auditoria de Secrets no Git Hook (detecta token sbp_ antes do push)
     Bloqueia commit localmente se padrão de token Supabase for detectado. Complementa G-5.
     Pergunta: Ferramenta de processo — sem impacto direto em Ingrid.

N-4: View SQL snapshot_ingrid_loop6_golden (102 registros imutáveis)
     View imutável com os 102 registros de performance de Loop 6 — preserva business case (E-4) mesmo se migração multi-tenant corromper associações.
     Pergunta: O business case da Ingrid (prova social para R$97/mês) está sólido o suficiente para o próximo pitch?

N-5: Pulse Check Analógico Pós-Automático
     Se cron job disparar com sucesso, Painel Eduardo emite: "Sistema enviou. Mande um '👍' manual 2 horas depois." IAH: sistema faz o trabalho pesado, humano valida o calor.
     Pergunta: Ingrid percebe a diferença entre mensagem automática e mensagem pessoal do Eduardo?
...
DECISÕES OBRIGATÓRIAS NESTE LOOP (Ingrid Loop 7):
  ► D1: DEPLOY-WATCH — supabase login + deploy F-4/F-6 executado? Smoke test válido?
  ► D2: RLS-WATCH — test_tenant_isolation.ps1 passou? Gate 7.2 desbloqueado?
  ► D3: PITCH-WATCH — Ingrid verbalizou progresso? Janela R$97/mês aberta?
  ► D4: GITHUB-WATCH — GitHub Pages unblocked? Push automático funcionando?
  ► D5: M-4 AJUSTE — Link demo só gráficos (sem conteúdo das questões) — aprovar restrição?
  ► D6: E-4 TIMING — Eduardo planta semente "quando você passar..." nesta semana?
