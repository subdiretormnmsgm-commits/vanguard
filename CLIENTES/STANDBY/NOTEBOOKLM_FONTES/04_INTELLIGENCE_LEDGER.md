...
**Ferramentas (P-146) [PENDENTE DE BUILD -- registrado no PENDENTES]:** (a) guard PreToolUse em Bash que nega gravacao em caminho protegido sem flag/[VEREDITO-DIRETOR]; (b) teste determinista que prova que file_protection_guard.ps1 consome a flag num Edit real; (c) se o consumo nao reproduzir, corrigir o wire do hook em settings.json.
...
**Ferramentas (P-146):** scripts/rclone_secrets_exclude.txt (filtro versionado, fonte unica) + verify_gdrive_freshness.ps1 corrigido. [A CONSTRUIR -- PENDENTES] guard que recusa  rclone sync  para destino externo sem --exclude-from do filtro de segredos (mesma filosofia P-180/P-181: mecanico, nao por memoria). Liga com HV-1 (credencial hardcoded), P-098, P-110, P-169/P-181 (G2), P-184 (superficies nao cobertas), P-146.
