...
## P-198 -- O ESTAGIO DO PIPELINE MORA EM DISCO (PIPELINE_STATE.json), NUNCA NA MEMORIA DO MUSCULO: O ORQUESTRADOR ENTREGA O COMANDO DO DIA JA PREENCHIDO, AVANCADO POR EVIDENCIA (2026-06-25) [VEREDITO-DIRETOR]
**Origem:** o Diretor pediu um orquestrador PERMANENTE e AUTOMATICO de toda a esteira de inteligencia de mercado (Executor Cowork -> Projetista -> Embaixador Digital), com gates/hooks/gatilhos por calendario que "ocorra sempre" e "nao tenha erros". Declaracao-regua: "preciso de ferramentas que facam que isso ocorra sempre. Percebi, ao longo do tempo, que memoria nao funciona. Deve ser automatico." A dor: o Musculo dependia de lembrar em que estagio cada produto estava e qual comando disparar na janela de calendario -- exatamente a falha-mae DEF-M-6 / P-180 / P-181 / P-188.
**Principio:** o ESTAGIO de cada produto (S0_ACUMULANDO .. S6_RETROALIMENTADO, mais S2b_RETIDO) e um FATO que mora em disco -- `PENTALATERAL_UNIVERSAL/INTELLIGENCE_HUB/PIPELINE_STATE.json`, fonte UNICA da verdade -- nunca na memoria do Musculo. O avanco de estagio segue a mesma isomorfia do P-087 ([RESOLVE:] em commit): estado PROVADO por evidencia em disco (campo `evidencia` apontando o artefato real do estagio), nao por o Musculo "lembrar" onde o produto esta. O orquestrador (`pipeline_orquestrador.ps1`) le o estado + a grade de calendario (`comandos_ativacao_atores.json`) e, para a data de hoje, monta a ACAO DO DIA por produto com o comando JA PREENCHIDO dos placeholders a partir do disco -- placeholder sem fonte vira `[AGUARDA: ...]` (Mandato 8 / GATE DE FATO, NUNCA inventa). Plugado no `session_start` como ponto de carga INCONDICIONAL: a regra vira mecanismo na fonte, nao lembrete (mesma logica de P-180/P-181/P-188).
**Limites de papel preservados (load-bearing):** (a) GATE DE PAPEL (CLAUDE.md item 42) -- o orquestrador rastreia ESTAGIO MECANICO, NAO decide/classifica/prioriza nicho; o avanco S2->S3 e S2b->S3 exige veredito do Diretor (Portoes 1 e 2 intactos). (b) GATE DE DATA COWORK -- comando so aparece na janela de calendario do ator (grade fixa) ou sob gatilho do seu portao. (c) P-124 -- read-only sobre o canonico: detecta deriva (produto em disco fora do PIPELINE_STATE) e ALERTA, nunca escreve. (d) Invariante testado: produto RETIDO (S2b) mostra o que FALTA, jamais vaza o comando de disparo ao Embaixador Digital.
**Fix aplicado:** 4 commits -- 5ad6afd (plano writing-plans + PROJETISTA/INBOX), 345682d (PIPELINE_STATE.json), 4ff760f (orquestrador + prova de datas `test_orquestrador.ps1` 8/8 VERDE), 5d641fc (session_start injeta o painel "ACAO DO DIA POR PRODUTO"). Fail-safe: qualquer erro -> exit 0 sem saida (nunca trava o session_start; espelha `ativacoes_do_dia.ps1`). Code-review R-05 (subagente) Ready-to-merge = Yes, 0 criticos/importantes; minors aplicados (comentario do ponto cego executor_cowork sem janela fixa; teste do invariante RETIDO; reconciliacao do plano com o disco superior). PIPELINE_STATE.json e `.json` -> invisivel ao `sync_vanguard_docs.ps1` (so .md/.txt) -> sync pos-build = 117 verificados / 0 falhas de hash / nenhum orfao novo (P-033).
**Aplica-se a:** todo produto da esteira de inteligencia de mercado (Cowork -> Projetista -> Embaixador Digital), toda sessao, com ou sem Loop. Liga com P-087 (estado provado por evidencia, nao memoria), P-180/P-181/P-188 (regra como mecanismo no ponto de carga incondicional), item 42/GATE DE PAPEL (Musculo nao decide nicho), GATE DE DATA COWORK (ativacao so na janela), Mandato 8/GATE DE FATO ([AGUARDA] sem fonte), P-124 (read-only, deriva so alerta), P-194 (modo corrente = inteligencia de mercado), DEF-M-6 (Musculo reativo). Ponto cego conhecido e documentado no codigo: produto cujo proximo ator seja `executor_cowork` (sem janela fixa na grade) cai em "AGUARDA JANELA" ate ganhar gatilho proprio.
