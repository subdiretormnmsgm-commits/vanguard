...
- Detecta falhas antes do Músculo em situações de pressão (P-021 — detectado como padrão recorrente)
...
| Autonomia do sistema | 6.0/10 | V28 = primeira camada real de autonomia |
| Base de clientes | 7.0/10 | 2 projetos ativos (Valdece + Ingrid), 0 em captação |
| Receita | 4.0/10 | R$5k comprometido, MRR não confirmado |
| Infraestrutura técnica | 8.0/10 | n8n + EasyPanel + GitHub + Supabase operacionais |

**CHURN-WATCH Interno:** N/A (sem clientes novos em pipeline)

### Hipóteses Ativas sobre o Sistema

| ID | Hipótese | Status |
|----|----------|--------|
| H-V1 | O Diretor consegue operar em modo CEO (deliberação) em vez de fundador (execução) | MAIS PROVÁVEL — V28 é o primeiro teste real |
| H-V2 | Hermes Agent consegue executar graus B/C com nível de erro aceitável (<5%) | INCERTA — não testada, 7 dias shadow mode para calibrar |
...
**Diferencial incopiável:** INTELLIGENCE_LEDGER + deficiências mapeadas + loop composto  
