# QUESTIONÁRIO DE DISCOVERY — CLIENTE
**IAH Factory — Quadrilateral** | Confidencial

---

> Este questionário leva aproximadamente 10 minutos a preencher.
> As suas respostas são confidenciais e serão usadas exclusivamente
> para desenhar uma solução adequada ao seu negócio.
> Não existe resposta errada — quanto mais concreto, melhor o resultado.

---

## BLOCO A — O SEU NEGÓCIO

**A1. Nome da empresa e sector de actividade:**
_______________________________________________

**A2. Qual é o seu cargo / papel na empresa?**
_______________________________________________

**A3. Há quanto tempo está no mercado?**
[ ] Menos de 1 ano   [ ] 1–3 anos   [ ] 3–10 anos   [ ] Mais de 10 anos

**A4. Qual é a receita mensal aproximada da empresa hoje?**
[ ] Pré-receita (ainda não fatura)
[ ] Até R$10.000/mês
[ ] R$10.000 – R$50.000/mês
[ ] R$50.000 – R$200.000/mês
[ ] Acima de R$200.000/mês

---

## BLOCO B — O PROJECTO

**B1. O que pretende construir?**
*(site, app, sistema interno, loja online, automação, outro — descreva com as suas palavras)*

_______________________________________________
_______________________________________________

**B2. Qual é o problema principal que este projecto resolve?**
*(o que está a custar dinheiro ou tempo por não existir)*

_______________________________________________
_______________________________________________

**B3. O que acontece se não resolver este problema nos próximos 3 meses?**
_______________________________________________

**B4. Já tentou resolver este problema antes? Como?**
[ ] Não — é a primeira vez que procuro solução
[ ] Sim — tentei: _______________________________
[ ] Sim — contratei outra empresa. O que correu mal: _______________

---

## BLOCO C — O CLIENTE FINAL E O VOLUME

**C1. Quem vai usar o produto que vamos construir?**
[ ] Os meus clientes finais (B2C)
[ ] Outras empresas (B2B)
[ ] A minha equipa interna
[ ] Todos os anteriores

**C2. Quantos utilizadores activos espera ter no primeiro mês após o lançamento?**
[ ] Menos de 100
[ ] 100 – 1.000
[ ] 1.000 – 10.000
[ ] Mais de 10.000

**C3. Qual é o ticket médio por cliente/venda/serviço hoje?**
R$ __________ por __________ (ex: R$150 por consulta / R$2.000 por contrato)

**C4. Quantos clientes ou transacções tem por mês actualmente?**
__________ por mês

---

## BLOCO D — O MOMENTO DE SUCESSO (FIRE EVENT)

**D1. Se um utilizador entrar no seu produto e fizer UMA coisa que significa que o seu negócio funcionou — qual é essa coisa?**

*(Escolha a principal)*

[ ] Comprou um produto ou serviço
[ ] Agendou uma consulta / reunião / visita
[ ] Pediu um orçamento
[ ] Entrou em contacto pelo WhatsApp
[ ] Registou-se / criou uma conta
[ ] Assinou uma subscrição
[ ] Outro: _______________________________________________

**D2. Hoje, quantas vezes por mês esse evento acontece?**
__________ vezes/mês

---

## BLOCO E — ESTADO ACTUAL E RECURSOS

**E1. O que já existe? (marque tudo o que se aplica)**
[ ] Site ou página online
[ ] Sistema / software em uso
[ ] Design / identidade visual criada
[ ] Domínio registado
[ ] Contas de serviços pagos (hospedagem, CRM, email marketing, etc.)
[ ] Base de dados de clientes
[ ] Código antigo / sistema legado

**E2. Descreva brevemente o que já existe:**
_______________________________________________

**E3. Qual é a stack técnica actual (se souber)?**
_______________________________________________

---

## BLOCO F — URGÊNCIA E INVESTIMENTO

**F1. Existe uma data, evento ou pressão que define quando precisa estar pronto?**
[ ] Não — sem prazo definido
[ ] Sim — motivo: _______________________________
[ ] Data desejada: _______________

**F2. Como classifica a urgência deste projecto?**
[ ] Exploratório — ainda estou a perceber o que quero
[ ] Normal — preciso nos próximos 2–3 meses
[ ] Urgente — preciso em menos de 1 mês
[ ] Crítico — está a custar dinheiro agora

**F3. Qual é o orçamento disponível para este projecto?**
[ ] Até R$2.000
[ ] R$2.000 – R$8.000
[ ] R$8.000 – R$25.000
[ ] R$25.000 – R$80.000
[ ] Acima de R$80.000
[ ] Ainda não definido — aguardo proposta

**F4. Quem toma a decisão final de avançar com o investimento?**
[ ] Eu próprio
[ ] Eu e um sócio / parceiro
[ ] Preciso de aprovação de terceiros — quem: _______________

---

## BLOCO G — EXPECTATIVAS

**G1. O que define, para si, que este projecto foi um sucesso?**
*(em termos de negócio — não em termos técnicos)*

_______________________________________________
_______________________________________________

**G2. Qual é o seu maior medo em relação a este tipo de projecto?**
*(ex: não ser entregue a tempo, custar mais do que o esperado, não ser usado pelos clientes)*

_______________________________________________

**G3. Já trabalhou com agências ou programadores antes?**
[ ] Não — é a primeira vez
[ ] Sim — correu bem
[ ] Sim — correu mal: _______________________________

---

## BLOCO H — CONTEXTO ADICIONAL

**H1. Há algo que considera importante partilhar e que as perguntas anteriores não cobriram?**

_______________________________________________
_______________________________________________

**H2. Como nos conheceu?**
[ ] Indicação de: _______________
[ ] Redes sociais
[ ] Pesquisa online
[ ] Evento / networking
[ ] Outro: _______________

---

*Obrigado pelo seu tempo. Entraremos em contacto em até [X] horas úteis com uma análise inicial.*

*IAH Factory — Quadrilateral | [email do operador] | [WhatsApp do operador]*
