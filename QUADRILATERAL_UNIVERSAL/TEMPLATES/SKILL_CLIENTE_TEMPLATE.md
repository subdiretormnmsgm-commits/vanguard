...
description: Skill do projecto [NOME DO CLIENTE] — gerada pelo Auditor (NotebookLM) para o Músculo (Claude Code). Activar ao iniciar qualquer sessão deste projecto.
---

# SKILL — [NOME DO CLIENTE / PROJECTO]
...
**Objectivo desta iteração:** [1 frase — o que o Músculo entrega]
...
Na iteração V[X], foi construído **[módulo]** com objectivo **[Y]**.
Reutilizar a estrutura de `[ficheiro:linha]` — economiza [estimativa].
Não reconstruir do zero.

Na iteração V[X], foi construído **[módulo 2]** com objectivo **[Y2]**.
Adaptar `[ficheiro]` para o novo contexto — [o que muda].
...
- **[padrão 1]** → resultado: [impacto concreto]. Expandir este padrão neste projecto.
- **[padrão 2]** → resultado: [impacto]. Aplicar directamente.
...
Baseado em [N] projectos similares ([lista de tipos]), o padrão que emerge é:

**O que sistematicamente funciona:** [insight específico com dados]
**O que sistematicamente falha:** [insight específico]
**O que este projecto tem de diferente:** [o que pode mudar o padrão]
...
*Actualizar ao fechar a iteração com novos padrões identificados*
