...
def clonar_projeto(
    ...
    (projeto_dir / "sentinel_config.json").write_text(
        json.dumps(sentinel, indent=2, ensure_ascii=False), encoding='utf-8'
    )
    ...
    (projeto_dir / "knowledge_graph.json").write_text(
        json.dumps(kg, indent=2, ensure_ascii=False), encoding='utf-8'
    )
...
"""
    (projeto_dir / "INTELLIGENCE_LEDGER.md").write_text(ledger_md, encoding='utf-8')
...
|------|--------|---------|----------|-----------|
"""
    (projeto_dir / "DECISOES_PENDENTES.md").write_text(decisoes, encoding='utf-8')

    # ── README.md ────────────────────────────────────────────────────────────
    readme = _criar_readme_projeto(cliente, slug, nicho, camada)
    (projeto_dir / "README.md").write_text(readme, encoding='utf-8')
